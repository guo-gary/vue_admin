		...
		<el-row>
			<el-col :span="options.span" v-for="item in options.list">
				<el-form-item :label="item.label" :prop="item.prop">
					...
					<!-- 新增 textarea 处理 -->
					<el-input v-if="item.type === 'textarea'" type="textarea" v-model="form[item.prop]" :rows="4"
						:disabled="item.disabled" :placeholder="item.placeholder" show-word-limit></el-input>
					...
					<el-upload v-else-if="item.type === 'upload'" class="avatar-uploader" :action="action"
...
import { getAction, getFile ,getUploadFile} from "@/api/file";
// import httpInstance from "@/utils/http";
// const baseURL = httpInstance.defaults.baseURL;
const action = getUploadFile;
console.log(action);

const { options, formData, edit, update, add } = defineProps({
	options: {
...
const handleAvatarSuccess = (response: Request, uploadFile) => {
	const { data } = response;
	form.value.circleAvatar = getFile(data);
	form.value.postImgUrls = getFile(data);
};
